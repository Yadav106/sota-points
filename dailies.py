...
def daily_handler():
    daily_screen = Screen(
        ...
        [
            ...
            ["get current week stats", get_current_week_points]
        ]
    )
...
def add_entry():
    with get_db_connection() as conn:
        ...
        if writers_id is None:
            print("[red]Unexpected error occured (writers_id is none in add_entry for dailies)")
        ...
        try:
            cursor.execute('''
                INSERT INTO dailies (writer_id, points, date) values ((?), (?), (?))
            ...
            print(f"[red]Error occured while adding daily points for user {name}")
...
def remove_entry():
    ...
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT writer_id, points from dailies where id = (?)
            ''', (id, ))
...
def get_entries():
    ...
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT dailies.id, writers.name, dailies.date, dailies.points
                FROM dailies
                JOIN writers ON dailies.writer_id = writers.id
                ORDER BY dailies.date DESC
                LIMIT ?;
            ''', (n, ))

            daily_stats = cursor.fetchall()

            table = Table(title="Daily Stats")
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute('''
                ...
                INNER JOIN dailies on dailies.writer_id = writers.id
                WHERE date >= ?
                GROUP BY name
...
def get_current_week_points():
    try:
        ...
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT writers.name, SUM(dailies.points) as weekly_points
                FROM writers
                INNER JOIN dailies ON dailies.writer_id = writers.id
                WHERE dailies.date BETWEEN ? AND ?
                GROUP BY writers.id
                ORDER BY weekly_points DESC
            ''', (start_of_week_str, end_of_week_str))

            week_points = cursor.fetchall()

            table = Table(title=f"Points for Week of {start_of_week.strftime('%B %d, %Y')}")
            table.add_column("Writer", style="cyan", no_wrap=True)
            table.add_column("Points Earned", style="cyan", no_wrap=True)
        ...
        print(f"[red]Error while fetching points for the current week: {ex}")
        Prompt.ask("[blue]Press enter to continue")
