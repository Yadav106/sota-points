...
def short_story_handler():
    short_story_screen = Screen(
        "Short Stories",
        [
...
def add_entry():
    with get_db_connection() as conn:
        ...
        if writers_id is None:
            print("[red]Unexpected error occured (writers_id is none in add_entry for short_stories)")
        ...
        try:
            cursor.execute('''
                INSERT INTO short_stories (writer_id, points, date) values ((?), (?), (?))
...
def remove_entry():
    ...
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT writer_id, points from short_stories where id = (?)
            ''', (id, ))
            ...
            cursor.execute('''
                DELETE from short_stories where id = (?)
            ''', (id, ))
...
def get_entries():
    ...
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT short_stories.id, writers.name, short_stories.date, short_stories.points
                FROM short_stories
                JOIN writers ON short_stories.writer_id = writers.id
                ORDER BY short_stories.date DESC
                LIMIT ?;
            ''', (n, ))
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            cursor.execute('''
                ...
                INNER JOIN short_stories on short_stories.writer_id = writers.id
                WHERE date >= ?
                GROUP BY name
